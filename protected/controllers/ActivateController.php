...
class ActivateController extends Controller{
    ...
    public function actionViciUser($identification)
    {
        ...
        if($model){
            //update the status
            $model->status = VicidialRemoteAgents::ACTIVE_STATUS;
            if($model->save()){
                Yii::app()->user->setFlash("success", "Viciuser $identification is now activated");
            }else{
                Yii::app()->user->setFlash("error", "We cant activate $identification.");
            }
